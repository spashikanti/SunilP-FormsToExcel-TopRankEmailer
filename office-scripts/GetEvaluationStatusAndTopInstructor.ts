...
function main(workbook: ExcelScript.Workbook) {
    ...
    const deadlineCellValue = settingsSheet.getRange("B3").getValue();

    // Convert deadline into a usable Date object
    // This handles both Excel serial and ISO string formats
    let deadlineDate: Date;

    if (typeof deadlineCellValue === "object" && deadlineCellValue !== null) {
        // Excel sometimes returns an object with date info
        deadlineDate = new Date(deadlineCellValue.toString());
    }
    else if (typeof deadlineCellValue === "number") {
